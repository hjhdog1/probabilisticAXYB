%% Generate Figure 7 & 8
% This code runs for 5 minutes (or about 10 mins when SGO is activated for distance minimization)
...
methods = {'Park', 'Li', 'Shah', 'geometric', 'prob'};
...
for i = 1:nExp
    ...
    % generate data
    [A,B] = generateABData_SE3(X_true, Y_true, nMeas, noiseLevel_SO3, 1, noiseLevel_trans, 0.0, 'G', noiseAPosition, noiseBPosition);   % last M pairs of (A,B) are outliers.
...
%                 [~,handEyeHT.TabbZc1,base2grid.TabbZc2,handEyeHT.TabbZc2,time.TabbZc1,time.TabbZc2] = method.AXZB(BasePoseIntcpCoords,gridPoseInCameraCoords,initial,Config) ;
    ...
    %% Store Results
    for j = 1:nMethods
        distX_SO3(i,j) = norm(so3(X{i,j}(1:3,1:3) * X_true(1:3,1:3)'));
        distY_SO3(i,j) = norm(so3(Y{i,j}(1:3,1:3) * Y_true(1:3,1:3)'));
        distX_trans(i,j) = norm(X{i,j}(1:3,4) - X_true(1:3,4));
        distY_trans(i,j) = norm(Y{i,j}(1:3,4) - Y_true(1:3,4));
    end
